...
export default function CartPage() {
  ...
  const deliveryCharge = subtotal >= 1000 ? 60 : 80;
          ...
          <div className="flex-1">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="p-6 md:p-8 flex flex-col gap-6 md:gap-8">
                {items.map((item) => (
                  <div key={`${item.productId}-${item.variantId ?? "default"}`} className="flex flex-col sm:flex-row gap-4 sm:gap-6 border-b border-gray-100 pb-6 md:pb-8 last:border-0 last:pb-0">
                    {/* Item Image */}
          ...
          <div className="w-full lg:w-[380px] shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8 sticky top-24">
              ...
              <div className="border-t border-gray-100 pt-4 mb-8">
                ...
                <p className="text-xs text-gray-400 mt-1 text-right">Taxes included • Cash on Delivery</p>
              </div>
              ...
              <div className="mt-4 flex items-center justify-center gap-2 text-xs text-gray-500 font-medium">
                <span className="w-2 h-2 rounded-full bg-green-500" /> Secure Checkout · Cash on Delivery
              </div>
            </div>
          </div>
        </div>
